...
class bDb {
	...
	function bDb() {
		global $my_db;
		$this->link = mysql_connect($my_db['host'],$my_db['user'],$my_db['pass']); // Connect to the MySQL server
	...
	function query($query_str,$multi=true) {		
		...
		}elseif(preg_match("/^\\s*(create|alter) /i",$query_str)){
			//return the result
			return $this->result;
		} else {
	...
	function show_error($error_msg) {		global $debug_mode;
	if($debug_mode) {
	echo $error_msg;
	} else {
	return;
