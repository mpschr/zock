	...
	//saveit!
		...
		if($_POST['formname']=='phase2'){
			...
			if($data['nextstep']==1){
				...
				if(stristr($_SERVER['HTTP_REFERER'], 'ssubmenu=settings&ev='.$data['id'])){
					...
					//do the queries
					if($db->query($query2)){
						echo '<br/>newtable: '.GT_('general_savedok');
						if($data['stake_mode'] == 'permatch'){
							$data['eve']=$data['id'];
						...
						echo '<p />query->'.$query2;
						echo '<br/>newtable: '.GT_('general_savednotok');
		...
		if($db->query($query)){
			if($data['formname']=='phase1'){
				...
				echo '<br/>eventsettings: '.GT_('general_savedok').'<br>';
				echo GT_('general_redirect');
				redirect($rlink.$_POST['form'], 3);
			}
		}else{
			echo '<br/>eventsettings: '.GT_('general_savednotok');
			echo '<p>query->'.$query;
		}
	}
...
//============= activate an event
	...
	if(stristr($_SERVER['HTTP_REFERER'], 'ssubmenu=settings&ev='.$data['ev'])){
		$query = "UPDATE ".PFIX."_events 
				...
				WHERE id = '".$data['ev']."';";
		if($db->query($query)){
			echo GT_('general_savedok');
			redirect($_SERVER['HTTP_REFERER'],3,1);
		}else{
			echo GT_('general_savednotok').'<br/>';
			echo $query;
...
//========== arrangematchdays
	...
	echo GT_('general_updating');
	echo GT_("This is stupid!");
	$data=$_POST;
