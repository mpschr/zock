...
if ($rows == 0) {
 ...
 //SELECT DISTINCT FROM_UNIXTIME( time, '%d.%m.%Y' ) FROM zock_event_9
	...
	foreach($listsource as $u => $r){
        ...
        foreach($rts as $rt)
            $tooltip .= '<br/>'.$rt['home'].' - '.$rt['visitor'].': <font class=&quot;ow_'.getResultCSSClass($evinfo,$rt['score_h'],$rt['score_v'],$rt[$u.'_h'],$rt[$u.'_v']).'&quot;>'
			.$rt[$u.'_h'].':'.$rt[$u.'_v'].'</font>';
            #$tooltip .= '<br/>'.$rt['home'].' - '.$rt['visitor'].': <font >'.$rt[$u.'_h'].':'.$rt[$u.'_v'].'</font>';
	    $tooltip .= '<br />'
